...
echo "Upload parameters.conf to mkt-hol-setup-"$cde_user
cde resource upload --name mkt-hol-setup-$cde_user --local-path setup/parameters.conf
...
cde job create --name mkt-hol-setup-$cde_user --type spark --mount-1-prefix jobCode/ --mount-1-resource mkt-hol-setup-$cde_user --application-file setup/setup.py --runtime-image-resource-name dex-spark-runtime-$cde_user
echo "Create job mkt-hol-setup-"$cde_user
